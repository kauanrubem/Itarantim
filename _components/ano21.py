...
def criar_grafico1_ano21(file_path):
    import pandas as pd
    import plotly.express as px
    ...
    filter_values = [
        ...
        'RECEITAS FUNDEB',
        'MÍNIMO A SER APLICADO NO MDE (25%)',
        'CONTRIBUIÇÃO PARA FORMAÇÃO DO FUNDEB (20%)',
...
def criar_grafico2_ano21(file_path):
    import pandas as pd
    import plotly.express as px
    from dash import dcc
...
def criar_grafico3_ano21(file_path):
    import pandas as pd
    import plotly.express as px
    from dash import dcc
...
def criar_grafico4_ano21(file_path):
    xls = pd.ExcelFile(file_path)
    ...
    excedente = max(0, value_percent - limit)
    texto_excedente = (
        f"Excedeu {excedente:.2f}% do limite de {limit}%"
        if excedente > 0 else
        f"Não excedeu o limite de {limit}%"
    )
    ...
    return html.Div([
        ...
        html.P(
            texto_excedente,
            style={'color': 'white', 'fontSize': '18px', 'textAlign': 'center', 'marginTop': '10px'}
        )
    ])

def criar_grafico5_ano21(file_path):
    # Carregar a planilha
    xls = pd.ExcelFile(file_path)
    ...
    filter_values = ['APLICAÇÃO MDE FONTE 01']
    df_filtered = df_resumo_indices_clean[df_resumo_indices_clean['Descrição'].isin(filter_values)]
...
def criar_grafico6_ano21(file_path):
    # Carregar a planilha
    xls = pd.ExcelFile(file_path)
    ...
    filter_values = ['APLICAÇÃO MDE 25% (TOTAL)']
    df_filtered = df_resumo_indices_clean[df_resumo_indices_clean['Descrição'].isin(filter_values)]

    # Pegar o valor percentual
    fundeb_value = df_filtered['Valor'].values[0]
    value_percent = fundeb_value * 100
    limit = 25  # limite de 25%
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'steps': [
                ...
                {'range': [limit, min(value_percent, 100)], 'color': '#F1948A'},  # vermelho claro
                {'range': [max(value_percent, limit), 100], 'color': 'lightgray'},
            ],
        },
        number={
...
def criar_grafico7_ano21(file_path):
    import pandas as pd
    import plotly.express as px
    from dash import dcc
...
def criar_grafico8_ano21(file_path):
    # Carregar a planilha
    xls = pd.ExcelFile(file_path)
    ...
    # Filtrar apenas o valor desejado
    filter_values = ['% APURADO NO EXERCÍCIO 2021']
    df_filtered = df_resumo_indices_clean[df_resumo_indices_clean['Descrição'].isin(filter_values)]

    fundeb_value = df_filtered['Valor'].values[0]
    value_percent = fundeb_value * 100
    limit = 54  # limite corrigido para 54%
...
def criar_grafico9_ano21(file_path):
    # Carregar a planilha
    xls = pd.ExcelFile(file_path)
    ...
    # Usar str.contains para maior robustez
...
ano21_layout = html.Div([
    dbc.Row([
        dbc.Col(dbc.Card(dbc.CardBody([
            html.H5("Índices Constitucionais e Legais", className="card-title"),
            criar_grafico1_ano21("dataset/ACOMPANHAMENTO ITARANTIM 2021.xlsx")
        ]), className="grafico-card"), width=4),

        dbc.Col(dbc.Card(dbc.CardBody([
            html.H5("Despesa Total em Aplicações na Educação", className="card-title"),
            criar_grafico2_ano21("dataset/ACOMPANHAMENTO ITARANTIM 2021.xlsx")
        ]), className="grafico-card"), width=4),

        dbc.Col(dbc.Card(dbc.CardBody([
            html.H5("Apuração em Saúde", className="card-title"),
            criar_grafico3_ano21("dataset/ACOMPANHAMENTO ITARANTIM 2021.xlsx")
        ]), className="grafico-card"), width=4),
    ]),
        ...
        dbc.Col(dbc.Card(dbc.CardBody([
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    criar_grafico4_ano21("dataset/ACOMPANHAMENTO ITARANTIM 2021.xlsx")
                ], width=4),

                dbc.Col([
                    html.H6("APURAÇÃO MDE FONTE 01", className="card-title"),
                    criar_grafico5_ano21("dataset/ACOMPANHAMENTO ITARANTIM 2021.xlsx")
                ], width=4),

                dbc.Col([
                    html.H6("APURAÇÃO MDE 25%", className="card-title"),
                    criar_grafico6_ano21("dataset/ACOMPANHAMENTO ITARANTIM 2021.xlsx")
                ], width=4),
            ])
    ...
    dbc.Row([
        dbc.Col(dbc.Card(dbc.CardBody([
            html.H5("APURAÇÃO LIMITE DESPESA COM PESSOAL 54%", className="card-title mb-4"),
            dbc.Row([
                dbc.Col([
                    html.H6("APURAÇÃO", className="card-title"),
                    criar_grafico7_ano21("dataset/ACOMPANHAMENTO ITARANTIM 2021.xlsx")
                ], width=6),

                dbc.Col([
                    html.H6("APURAÇÃO NO EXERCÍCIO 2021", className="card-title"),
                    criar_grafico8_ano21("dataset/ACOMPANHAMENTO ITARANTIM 2021.xlsx")
                ], width=6),
            ])
        ]), className="grafico-card-dupla"), width=8),

        dbc.Col(dbc.Card(dbc.CardBody([
            html.H5("PERCENTUAL DE APLICAÇÃO EM AÇÕES E SERVIÇOS PÚBLICOS DE SAÚDE 15%", className="card-title"),
            criar_grafico9_ano21("dataset/ACOMPANHAMENTO ITARANTIM 2021.xlsx")
